...
export async function bumpVersion(
    ...
    if (
        config.get(CONFIG_SKIP_PACKAGE_JSON) &&
        config.get(CONFIG_SKIP_ANDROID) &&
        config.get(CONFIG_SKIP_IOS)
    ) {
...
export async function bumpPackageJsonVersion(
    rootPath: string,
    type: BumpType
): Promise<BumpResult> {
...
export async function bumpAndroidVersion(
    rootPath: string,
    type: BumpType
): Promise<BumpResult> {
    const config = vscode.workspace.getConfiguration(
...
export async function bumpIOSVersion(
    rootPath: string,
    type: BumpType
): Promise<BumpResult> {
    const config = vscode.workspace.getConfiguration(
    ...
    let plistPath: string | null | undefined = config.get(
        CONFIG_IOS_INFO_PLIST_PATH
    );
    if (plistPath) {
