...
export class VersionCodeLensProvider implements vscode.CodeLensProvider {
    ...
    private providePackageJsonCodeLenses(document: vscode.TextDocument, token: vscode.CancellationToken): vscode.ProviderResult<vscode.CodeLens[]> {
        ...
        try {
            ...
            if (versionLineIndex !== -1) {
                ...
                const range = new vscode.Range(
                    position,
                    new vscode.Position(versionLineIndex, versionLineText.length)
                );
    ...
    private provideBuildGradleCodeLenses(document: vscode.TextDocument, token: vscode.CancellationToken): vscode.ProviderResult<vscode.CodeLens[]> {
        const codeLenses: vscode.CodeLens[] = [];
        const text = document.getText();
        const lines = text.split('\n');
    ...
    private addAndroidCodeLenses(codeLenses: vscode.CodeLens[], range: vscode.Range, versionName: string, versionCode: number): void {
    ...
    private provideIOSCodeLenses(document: vscode.TextDocument, token: vscode.CancellationToken): vscode.ProviderResult<vscode.CodeLens[]> {
        const codeLenses: vscode.CodeLens[] = [];
        const text = document.getText();
        const lines = text.split('\n');
        ...
        if (usesVariables) {
            try {
                ...
                if (fs.existsSync(pbxprojPath)) {
                    ...
                    if (versionVarName) {
                        const versionMatch = pbxprojContent.match(new RegExp(`${versionVarName}\\s*=\\s*["']?([\\d\\.]+)["']?`));
                        if (versionMatch) {
    ...
    private addIOSCodeLenses(codeLenses: vscode.CodeLens[], range: vscode.Range, version: string, buildNumber: string): void {
        const patchVersion = bumpSemanticVersion(version, "patch");
        const minorVersion = bumpSemanticVersion(version, "minor");
        const majorVersion = bumpSemanticVersion(version, "major");
...
export function registerVersionCodeLensProvider(context: vscode.ExtensionContext): void {
    const codeLensProvider = new VersionCodeLensProvider();

    // Register the provider for package.json, build.gradle, and iOS files
