...
import { CONFIG_IOS_INFO_PLIST_PATH, INITIAL_SEMANTIC_VERSION } from '../constants';
...
export async function getCurrentVersions(): Promise<ProjectVersions> {
    ...
    try {
        ...
        if (fs.existsSync(packageJsonPath)) {
            versions.packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8')).version;
        }
        ...
        if (fs.existsSync(iosPath)) {
            ...
            const plistPath = config.get(CONFIG_IOS_INFO_PLIST_PATH)
                ? path.join(rootPath, config.get(CONFIG_IOS_INFO_PLIST_PATH) as string)
                ...
                if (plistUsesVariables) {
                    ...
                    if (pbxprojPath && fs.existsSync(pbxprojPath)) {
                        ...
                        const buildNumberMatch =
                            pbxContent.match(new RegExp(`${buildVarName}\\s*=\\s*(["']?)(\\d+)\\1`, 'i')) ||
                            pbxContent.match(new RegExp(`${buildVarName}\\s*=\\s*(\\d+)`, 'i'));

                        const versionMatch =
                            pbxContent.match(new RegExp(`${versionVarName}\\s*=\\s*(["']?)([^;"']+)\\1`, 'i')) ||
                            pbxContent.match(new RegExp(`${versionVarName}\\s*=\\s*([^;]+)`, 'i'));

                        if (buildNumberMatch && versionMatch) {
                            versions.ios = {
                                buildNumber: buildNumberMatch[buildNumberMatch.length - 1],
                                version: versionMatch[versionMatch.length - 1].replace(/['"]*/g, ''),
                            };
                        }
                    }
                } else {
